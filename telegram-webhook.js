...
/* V309: quieter Telegram UX.
   Limited Bean add/edit wizard now reuses one bot message instead of sending a new bot
   message for every step. User input messages are also deleted where Telegram permits it.
   Other callback-driven Limited Bean operations keep editing the current Telegram card. */
async function sendTelegramMessageV309(env, chatId, text, replyMarkup) {
  try {
    const body = { chat_id: chatId, text };
    if (replyMarkup) body.reply_markup = replyMarkup;
...
async function deleteTelegramMessageV309(env, chatId, messageId) {
  try {
...
async function editOrSendTrackedV309(env, chatId, messageId, text, markup) {
  if (messageId) {
    const edited = await editTelegramMessage(env, chatId, messageId, text, markup);
    if (edited && edited.ok !== false) return messageId;
  }
  const sent = await sendTelegramMessageV309(env, chatId, text, markup);
  return sent && sent.result && sent.result.message_id ? sent.result.message_id : messageId;
}

async function saveLimitedDraftMessageIdV309(env, chatId, draft, messageId) {
  if (!draft || !messageId || String(draft.messageId || "") === String(messageId)) return;
  draft.messageId = messageId;
  await saveLimitedDraftV183(env, chatId, draft);
}

function buildLimitedStepTextV309(draft) {
  const stepNo = Number(draft.step || 0);
  const step = LIMITED_BEAN_STEPS_V183[stepNo];
  const modeText = draft.mode === "edit" ? "編輯限定豆子" : "新增限定豆子";
  const lines = [];
  ...
  lines.push(buildLimitedDraftSummaryV183(draft, false));
  lines.push("");
  lines.push("提示：直接在下方輸入內容，這張 Telegram 卡片會自動更新，不會每一步再新增一條 Bot 訊息。");
  return lines.join("\n");
}

buildLimitedStepTextV183 = buildLimitedStepTextV309;
...
sendLimitedWizardPromptV183 = async function(env, chatId, draft) {
  ...
  const mid = await editOrSendTrackedV309(env, chatId, draft.messageId, buildLimitedStepTextV183(draft), buildLimitedStepMarkupV183(draft));
  await saveLimitedDraftMessageIdV309(env, chatId, draft, mid);
};

sendLimitedWizardConfirmV183 = async function(env, chatId, draft) {
  const mid = await editOrSendTrackedV309(env, chatId, draft.messageId, buildLimitedConfirmTextV183(draft), {
    inline_keyboard: [
      [{ text: "✅ 確認上傳", callback_data: "limited_wizard_confirm:x" }],
      [{ text: "⬅️ 上一步", callback_data: "limited_wizard_back:x" }],
      [{ text: "取消", callback_data: "limited_wizard_cancel:x" }]
...
handleLimitedWizardTextV183 = async function(env, message, text, draft) {
  ...
  // Keep the chat tidy. Telegram may refuse deletion in some clients/chats; ignore failures.
  await deleteTelegramMessageV309(env, chatId, message.message_id);

  if (value === "取消" || value.toLowerCase() === "cancel") {
    await clearLimitedDraftV183(env, chatId);
    await editOrSendTrackedV309(env, chatId, draft.messageId, "已取消限定豆子編輯。", buildCommandMenuMarkupV183());
    return json({ ok: true });
  }
  ...
  if (step.required && !value) {
    const mid = await editOrSendTrackedV309(env, chatId, draft.messageId, "這一欄必填。\n\n" + buildLimitedStepTextV183(draft), buildLimitedStepMarkupV183(draft));
    await saveLimitedDraftMessageIdV309(env, chatId, draft, mid);
    return json({ ok: true });
  }
...
const _handleLimitedMenuActionBeforeV309 = handleLimitedMenuAction;
handleLimitedMenuAction = async function(env, cq, data) {
  const chatId = cq.message && cq.message.chat ? cq.message.chat.id : env.TELEGRAM_CHAT_ID;
  const messageId = cq.message ? cq.message.message_id : null;
  if (!isAuthorizedTelegramChat(env, chatId)) return stop(env, cq, "沒有權限");
  ...
  if (action === "limited_wizard_back") {
    const draft = await getLimitedDraftV183(env, chatId);
    if (!draft) return stop(env, cq, "沒有正在編輯的豆子");
    draft.messageId = draft.messageId || messageId;
    draft.step = Math.max(0, Number(draft.step || 0) - 1);
    await saveLimitedDraftV183(env, chatId, draft);
    await sendLimitedWizardPromptV183(env, chatId, draft);
    return stop(env, cq, "已返回上一步");
  ...
  if (action === "limited_wizard_cancel") {
    const draft = await getLimitedDraftV183(env, chatId);
    const targetMessageId = (draft && draft.messageId) || messageId;
    await clearLimitedDraftV183(env, chatId);
    await editOrSendTrackedV309(env, chatId, targetMessageId, "已取消限定豆子編輯。", buildCommandMenuMarkupV183());
    return stop(env, cq, "已取消");
  }
  ...
  return _handleLimitedMenuActionBeforeV309(env, cq, data);
};
